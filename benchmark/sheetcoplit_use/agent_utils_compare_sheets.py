...
def compare_format_conditions(formatCondition1, formatCondition2, report):
    ...
    # 修改后
    if 'formula1' in format_properties.keys() and (type(formatCondition1) is not type(formatCondition2)) :
        format_properties['formula1'] = 0

    if not('formula1' in format_properties.keys() and (type(formatCondition1) is not type(formatCondition2))):
        if (hasattr(formatCondition1, 'Formula1') and not hasattr(formatCondition2, 'Formula1')) or (not hasattr(formatCondition1, 'Formula1') and hasattr(formatCondition2, 'Formula1')):
            format_properties['formula1'] = 0

        if (hasattr(formatCondition1, 'Formula1') and hasattr(formatCondition2, 'Formula1')):
            if formatCondition1.Formula1 != formatCondition2.Formula1:
                format_properties['formula1'] = 0
