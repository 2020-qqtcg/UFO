    ...
    for repeat_id in range(1, config["repeat"] + 1):
        ...
        with tqdm.tqdm(total=remaining_task_cnt, desc=f"Processing the remaining {remaining_task_cnt}/{num_tasks} results of repeat {repeat_id}") as pbar:
            for index, row in task_df.iloc[:].iterrows():
                if index + 1 in check_result["checked_list"]:
                    print("index + 1 in check_result['checked_list']")
                    continue
                ...
                if not os.path.exists(task_path):
                    print("no task_path: ",task_path)
                    continue

                # 新加的
                # 获取目录下所有 Excel 文件
                excel_files = [f for f in os.listdir(task_path) if f.endswith(('.xls', '.xlsx'))]
                if not excel_files:
                    # print("no excel_files: ", task_path)
                    continue  # 如果没有 Excel 文件，跳过该目录
                # 将_{repeat_id}删去
                res_path = os.path.join(task_path, f"{task_name}.xlsx") #Claude
                ...
                #     no_find_log_num=no_find_log_num+1
                #     print(no_find_log_num,"log_file not found: ",log_file)
                #     continue
                #
                ...
                # if os.path.exists(log_file) and res_file_exists:
                    ...
                    for gt_file in [x for x in os.listdir(gt_folder_this_task) if x.endswith('.xlsx') and "$" not in x]:
                        ...
                        print(gt, res_path, check_board["check_board"])
                        print(check_res)
                        print("----------------------------------------------")
                        """
                        Comparing.....................
